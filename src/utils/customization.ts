...
export function applySize(
  ...
  if (typeof size === 'string') {
    ...
    if (size.spacingScale) {
      element.style.setProperty('--spacing-scale', size.spacingScale.toString());
    }
  }
}
 ...
 */
export function applyCardDesign(element: HTMLElement, design: CardDesign): void {
...
export function applyAnimationConfig(
  ...
  element.setAttribute('data-enable-card-animations', finalConfig.enableCardAnimations ? 'true' : 'false');
  element.setAttribute('data-enable-chip-animations', finalConfig.enableChipAnimations ? 'true' : 'false');
  element.setAttribute('data-enable-action-highlight', finalConfig.enableActionHighlight ? 'true' : 'false');
  
  // Apply animation durations
  if (finalConfig.cardDealDuration) {
    element.style.setProperty('--card-deal-duration', `${finalConfig.cardDealDuration}ms`);
  }
  if (finalConfig.actionTransitionDuration) {
    element.style.setProperty('--action-transition-duration', `${finalConfig.actionTransitionDuration}ms`);
  }
}
...
export function validateThemeColors(colors: ThemeColors): {
  ...
  const contrastRatio = calculateContrastRatio(colors.textPrimary, colors.bgPrimary);
  if (contrastRatio < 4.5) {
    issues.push('Primary text does not have sufficient contrast with primary background');
  }
  
  // Check that card colors are readable
  const cardContrastRatio = calculateContrastRatio(colors.cardText, colors.cardBg);
  if (cardContrastRatio < 4.5) {
    issues.push('Card text does not have sufficient contrast with card background');
  }
