...
jest.mock('lucide-react', () => ({
  Play: ({ size, ...props }: any) => (
    <span data-testid="play-icon" data-size={size} {...props}>
      Play
    </span>
  ),
  Pause: ({ size, ...props }: any) => (
    <span data-testid="pause-icon" data-size={size} {...props}>
      Pause
    </span>
  ),
  SkipBack: ({ size, ...props }: any) => (
    <span data-testid="skip-back-icon" data-size={size} {...props}>
      Previous
    </span>
  ),
  SkipForward: ({ size, ...props }: any) => (
    <span data-testid="skip-forward-icon" data-size={size} {...props}>
      Next
    </span>
  ),
  RotateCcw: ({ size, ...props }: any) => (
    <span data-testid="rotate-icon" data-size={size} {...props}>
      Reset
    </span>
  ),
}));
...
describe('Controls Component', () => {
  ...
  describe('Validation and Error Handling', () => {
    ...
    test('handles invalid totalActions gracefully', () => {
      ...
      expect(consoleSpy).toHaveBeenCalledWith(expect.stringContaining('totalActions must be >= 1'));
